...
# Coping Files
This challenge is to copy the `/flag` file into `/tmp/hack-the-planet` 

# The Solve
I used the `cp` command to copy the `/flag` file into the `/tmp/hack-the-planet` directory without deleting the original file. After copying, I ran `/challenge/check` to verify the solution and receive the flag.
...
pwn.college{4p0Cj1xQiF_zq6k1c987OnYFzcB.0lNxQTMywiMwEzNzEzW}
```

# What I learnt 
I learned the key difference between `cp` and `mv`, where `cp` preserves the original file while `mv` removes it from its source. I understood how to use the `cp` command to copy files into specific directories safely. I practiced using absolute file paths to manage files from any location in the system and learned how to use a checker command. Overall, this helped me avoid accidental data loss during command-line operations.
